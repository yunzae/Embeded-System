...
//@TODO - 1 Set the clock, (//) ??? ??ø? ????? ??? ???? ???? ????? ?? ?????? ??????y? 
        ...
        RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE2_DIV2; // 28MHz 를 나누기 2 / SART1->BRR |= 0x1E6; 
        /* PCLK1 = HCLK */
...
//@TODO - 4 GPIO Configuration
    ...
    /*Reset Port D CRH - User S1 Button */
    GPIOD->CRH &= ~(GPIO_CRH_CNF11 | GPIO_CRH_MODE11);
    /* MCO Pin Configuration */
    GPIOA->CRH |= (GPIO_CRH_CNF8_1 | GPIO_CRH_MODE8_1 | GPIO_CRH_MODE8_0);
    /* USART Pin Configuration */
    GPIOA->CRH |= (GPIO_CRH_CNF9_1 | GPIO_CRH_MODE9_1 | GPIO_CRH_MODE9_0) | GPIO_CRH_CNF10_1;
...
//@TODO - 7: Parity : None
    // originally PCE 0 value
    // originally PS 0 value 
...
int main() {
    ...
    volatile unsigned int idr=(uint32_t)GPIOD->IDR;
    while (1) {
